...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_full_update_recipe(self):
        """Test updating recipie with put"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
...
class RecipeImageUploadTests(TestCase):
    """Test uploading image file to recipe"""
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create_user(
            'testit@help.com'
    ...
    def test_filter_recipes_by_tag(self):
        """Test return recipies with specific tags"""
        recipe1 = sample_recipe(user=self.user, title='Thai Vegetable')
        recipe2 = sample_recipe(user=self.user, title='TurDuckin')
        tag1 = sample_tag(user=self.user, name='Vegan')
    ...
    def test_filter_recipies_by_ingredient(self):
        """Test return recipies with specific ingredients"""
        recipe1 = sample_recipe(user=self.user, title='Tacos')
        recipe2 = sample_recipe(user=self.user, title='Burrito')
        ...
        recipe3 = sample_recipe(user=self.user, title='FCasserole')
